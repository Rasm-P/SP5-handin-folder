...
- Create two local databases to be used for local development (replace persons with a name that makes sense for your project):
  - `persons`
  - `persons_test`
- *(We suggest you always follow the naming pattern used above (persons and persons_test) for your databases)*
- Create a REMOTE database on your Droplet with the same name as your dev-database (persons above)
- **Important:** Locate the file **.travis.yml** in the root, and change the script `CREATE DATABASE person_test;`to use the SAME name as your local test-database (persons_test) above
